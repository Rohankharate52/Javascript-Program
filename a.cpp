...
void man(char *name)
{
    ...
    else if(strcmp(name,"Iseek") == 0)
    {
        ...
        printf("Usage : Iseek File_ Name ChangeInOffset StartPoint\n");
    }
...
void DisplayHelp()
{
    ...
    printf("read : To Read the contentsfrom file\n");
...
int OpenFile(char *name, int mode)
{
    ...
    UFDTArr[i].ptrfiletable = (PFILETABLE) malloc(size(FILETABLE));
    if(UFDTArr[i].ptrfiletable== NULL)
    return -1;
    UFDTArr[i].ptrfiletable->count = 1;
...
int stat_file(char *name)
{
    ...
    printf("\n------------Statistical Information about file---------- An");
    printf("File name : %s\n",temp->FileName);
...
int main()
{
    ...
    CreateDILB();//yyyyyyyyyyyyyyyyyyyyyyyyydone
    ...
    while(1)
    {
        ...
        else if(count == 4 )
        {
            if(strcmp(command[0],"Iseek")==0)
            {
                ...
                if(fd == -1)
                {
                    printf("ERROR : Icorrect parameter\n");
                    continue;
                }
